...
class DPendulum:
    ...
    def __init__(self, nbJoint = 1, nu=11, vMax=5, uMax=5, dt=0.2, ndt=1, noise_stddev=0):
        ...
        self.DU = 2*uMax/(nu-1)     # modified from nu to (nu - 1) from REPO - this is the discretization resolution for joint torque
    ...
    def reset(self,x=None): # come la repo
    ...
    def render(self): #come la repo
        self.pendulum.render()
        time.sleep(self.pendulum.DT)

    def dynamics(self, x, iu): # ok come la repo (x non usata SI PUO' TOGLIERE)
        u   = self.d2cu(iu)
        self.xc,cost = self.pendulum.step(u)
        return self.xc,cost
    
    def plot_V_table(self, V, q, dq, tabNum = 0): # ok similissimo alla repo
        ''' Plot the given Value table V '''
        import matplotlib.pyplot as plt
        
        plt.figure() # added by Nicolo'
        plt.pcolormesh(q, dq, V, cmap=plt.cm.get_cmap('Blues'))
        plt.colorbar()
        plt.title('V table %d ' % tabNum)
        plt.xlabel("q")
        plt.ylabel("dq")
        plt.show()
        
    def plot_policy(self, pi, q, dq, tabNum = 0): # ok similissimo alla repo
        ...
        plt.colorbar()
        plt.title('Policy table %d' % tabNum)
        plt.xlabel("q")
        plt.ylabel("dq")
        plt.show()