...
import time
import random
from auxiliary_func import dyn_forNbigger_thanOne, compute_V_pi_from_Q, update, render_greedy_policy, action_selection, np2tf, trajectories
...
def dqn(env, gamma, nEpisodes, maxEpisodeLength, \
    ...
    replay_buffer       = []                                             # used for storing transitions and replaying experiences collected from interactions of the model with the environment
    hist_cost           = []                                             # list to keep track of cost-to-go history (for plotting)
    hist_x              = np.zeros([maxEpisodeLength, env.state_size()]) # list to keep track of state x (for plotting)
    hist_u              = np.zeros(maxEpisodeLength)                     # list to keep track of control u (for plotting)                               
